...
/**
 * Invoke a strategy's `normalizeParams` with string-valued inputs. The whole
 * point of these smoke tests is to verify each strategy coerces stringy params
 * (the shape Finder/UI hand off) to canonical numbers, so the input is
 * intentionally typed looser than `StrategyParams` and cast through `unknown`.
 * Asserts `normalizeParams` is defined (it is optional on `Strategy`).
 */
function normalizeParamsAsStringRecord(
    strategy: Strategy,
    ...
    return strategy.normalizeParams(params as unknown as StrategyParams);
}
...
describe("Synthetic Ratio Strategies Smoke Tests", () => {
    ...
    it("median_distance_bar_rejection executes and normalizes", () => {
        ...
        const norm = normalizeParamsAsStringRecord(median_distance_bar_rejection, { lookback: "10.5", zscoreThreshold: "2.5" });
    ...
    it("return_sign_streak_fade executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(return_sign_streak_fade, { lookback: "3.2", streakMin: "4.7" });
    ...
    it("cumulative_return_percentile_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(cumulative_return_percentile_reversion, { lookback: "20.1", pctlExtreme: "0.95" });
    ...
    it("return_zscore_extreme_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(return_zscore_extreme_reversion, { lookback: "15", zscoreThreshold: "2.2" });
    ...
    it("body_proportion_percentile_fade executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(body_proportion_percentile_fade, { lookback: "25", pctlExtreme: "0.80" });
    ...
    it("return_reversal_count_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(return_reversal_count_reversion, { lookback: "15.4", crossingMin: "8" });
        expect(norm.lookback).to.equal(15);
    ...
    it("decay_pressure_percentile_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(decay_pressure_percentile_reversion, { lookback: "35", pctlExtreme: "0.88" });
    ...
    it("close_location_streak_mean_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(close_location_streak_mean_reversion, { lookback: "4", streakMin: "4" });
    ...
    it("median_crossing_frequency_fade executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(median_crossing_frequency_fade, { lookback: "25", crossingMin: "6" });
        expect(norm.lookback).to.equal(25);
    ...
    it("volatility_zscore_reversion_trigger executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_zscore_reversion_trigger, { lookback: "25", volZThreshold: "2.3" });
        expect(norm.lookback).to.equal(25);
    ...
    it("volatility_percentile_reversion_fade executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_percentile_reversion_fade, { lookback: "30", volThreshold: "0.85", zThreshold: "1.9" });
    ...
    it("range_volatility_divergence_fade executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(range_volatility_divergence_fade, { lookback: "25", rangePctThreshold: "0.90", maxVolPercentile: "0.35" });
        expect(norm.lookback).to.equal(25);
    ...
    it("volatility_breakout_follow executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_breakout_follow, { lookback: "25", zThreshold: "1.9" });
        expect(norm.lookback).to.equal(25);
        expect(norm.zThreshold).to.equal(1.9);
    });
    ...
    it("historical_volatility_envelope_fade executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(historical_volatility_envelope_fade, { lookback: "25", multiplier: "2.5" });
        expect(norm.lookback).to.equal(25);
    ...
    it("atr_normalized_range_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(atr_normalized_range_reversion, { lookback: "20", shockPercentile: "0.95" });
        expect(norm.lookback).to.equal(20);
    ...
    it("volatility_skewness_exhaustion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_skewness_exhaustion, { lookback: "35", skewThreshold: "1.8" });
        expect(norm.lookback).to.equal(35);
    ...
    it("volatility_gated_close_acceptance executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_gated_close_acceptance, { lookback: "25", maxVolPercentile: "0.30", acceptanceThreshold: "0.75" });
        expect(norm.lookback).to.equal(25);
    ...
    it("volatility_autocorrelation_gated_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_autocorrelation_gated_reversion, { lookback: "30", acThreshold: "0.25", zThreshold: "1.8" });
        expect(norm.lookback).to.equal(30);
    ...
    it("compression_ratio_expansion_reversion executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(compression_ratio_expansion_reversion, { lookback: "25", compressThreshold: "0.25", rangePercentileMin: "0.75" });
        expect(norm.lookback).to.equal(25);
    ...
    it("volatility_weighted_momentum_autocorrelation executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volatility_weighted_momentum_autocorrelation, { lookback: "25", minAutoCorr: "0.3" });
        expect(norm.lookback).to.equal(25);
    ...
    it("entropy_decay_momentum_acceleration executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(entropy_decay_momentum_acceleration, { lookback: "25", entropyPercentileMax: "0.25" });
        expect(norm.lookback).to.equal(25);
    ...
    it("initiative_pressure_acceleration_follow executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(initiative_pressure_acceleration_follow, { lookback: "30", accThreshold: "0.35" });
        expect(norm.lookback).to.equal(30);
    ...
    it("close_acceptance_decay_weighted_drift executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(close_acceptance_decay_weighted_drift, { lookback: "20", decayFactor: "0.80" });
        expect(norm.lookback).to.equal(20);
    ...
    it("skewness_reversal_momentum_ignite executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(skewness_reversal_momentum_ignite, { lookback: "35", skewChange: "0.90" });
        expect(norm.lookback).to.equal(35);
    ...
    it("range_efficiency_regime_ignition executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(range_efficiency_regime_ignition, { lookback: "25", efficiencyMin: "0.55", rangePercentileMin: "0.80" });
        expect(norm.lookback).to.equal(25);
    ...
    it("volume_correlation_trend_follow executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(volume_correlation_trend_follow, { lookback: "25", correlationThreshold: "0.40" });
        expect(norm.lookback).to.equal(25);
    ...
    it("atr_adjusted_streak_momentum executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(atr_adjusted_streak_momentum, { lookback: "20", minStreak: "4" });
        expect(norm.lookback).to.equal(20);
    ...
    it("autocorrelation_crossover_momentum executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(autocorrelation_crossover_momentum, { shortLookback: "12", longLookback: "40" });
    ...
    it("wick_imbalance_thrust_continuation executes and normalizes", () => {
        ...
        expect(signals).to.be.an("array");
        const norm = normalizeParamsAsStringRecord(wick_imbalance_thrust_continuation, { lookback: "25", maxWickPercentile: "0.30", rocZThreshold: "1.5" });
        expect(norm.lookback).to.equal(25);
