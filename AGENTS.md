...
- Finder **Symbol Universe** runs in the Vite dev server via `lib/finder/server/finder-vite-plugin.ts`. Current-chart Finder remains in-tab. See `docs/finder-server-side.md`.
- Each selected entry strategy uses one sequential server request; `FinderManager.runUniverseFinder` merges the scalar survivors before the shared browser-side OOS pass. Polymarket scoring remains unsupported in Universe mode.
...
- **OOS pass runs in-browser**: the server runs IS and `FinderManager.runUniverseFinder` feeds the merged survivors into `applyUniverseOosValidationIfNeeded`. Datasets are fetched client-side via `loadUniverseDataset`.
...
- **Data slicing MUST be applied in the server loader wrapper**: the HTTP handler's `loadDatasetWithSlice` applies `sliceFinderDataWindow(data, options.dataSlice)` before evaluation. The browser OOS tail applies its complementary slice separately.
- **Stop MUST POST `/api/finder/stop`** during a Universe run and set `this.isCancelled = true` for browser-side OOS work.
...
- Validation habit after Server-Side Finder changes:
  ...
  - `..\..\..\node_modules\.bin\esno tests\feature-dom-contracts.spec.ts`
  - Manual smoke: start `NODE_OPTIONS=--max-old-space-size=16384 npm run dev`, run one and multiple strategies over 50 symbols, then 400 symbols. Confirm progress, Stop, diagnostics, merged sorting, and OOS filtering.
