...
- **Mine Prediction diagnostic** — the server HTTP endpoint and browser button were removed after the negative conclusion in `docs/mine-timing-validation-findings.md`. The standalone CLI `npm run diagnose:mine-prediction` (`scripts/diagnose-mine-prediction.ts`) is retained so the same question can be asked about a future strategy. Flags: `--sample-from`, `--sample-to`, `--direction-filter`, `--backtest-settings-file`, `--horizons`, `--sample-bars`, `--sample-step`. Do not re-add a UI/HTTP surface for this without first reading the findings doc; the test design (dense sampling n>100, direction filter, matched-horizon A/B) is the load-bearing part.
- **Portfolio Fit** — removed. The engine that used `oosLiftPct` as `edge%` was built on an input that the validation proved carries no predictive information. Do not reintroduce allocation advice from Mine Timing statistics.
...
- **F4 Disconnect-safe NDJSON streams**: all Batch streaming handlers wrap `beginNdjsonStream` in `createDisconnectSafeStream` (`lib/vite-http-utils.ts`). A disconnected Run keeps executing because `/status` can reattach and recover rows. Mine, Stability Mine, and OPEN_SCORE USD Replay cancel on disconnect because their streamed result is not recoverable; retained Batch artifacts remain available for retry. Do NOT pass `stream.write` directly to a long-running process function; use the wrapper's `.write`/`.end`.
