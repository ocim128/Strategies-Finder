...
- **F4 Disconnect-safe NDJSON streams**: all four Batch streaming handlers (Run, Mine, Stability Mine, Portfolio Fit) wrap `beginNdjsonStream` in `createDisconnectSafeStream` (`lib/vite-http-utils.ts`), which drops writes after `close`/`error` so a browser reload mid-run does not throw into the run loop. `/status` remains authoritative for reattach. Do NOT pass `stream.write` directly to a long-running process function; use the wrapper's `.write`/`.end`.
- **F5 runId-scoped Stop**: Batch now generates a browser `runId` (`BatchBacktestService.activeServerRunId`), sends it on `/run` and `/stop`, and the server scopes Stop to the matching run (`handleStopRequest(runId)` rejects a mismatched id without mutating ownership). A `pendingStopRunId` single-slot closes the Stop-before-ownership race. `runId` travels on `start`/`done`/`fatal` events and both `run` and `lastRun` status branches. Legacy unscoped Stop (empty/missing runId, or a server run with `runId: ""`) still cancels for backward compat with stale browser bundles — do NOT make the runId strictly required without a migration.
...
- **F7 PID-scoped orphan sweep**: artifact dirs are named `strategies-finder-batch-mine-<pid>-<createdAtMs>-<random>` and `sweepOrphanedMineArtifactDirs` reclaims a dir only when its PID is provably dead (`process.kill(pid, 0)` throws) OR it is older than `ORPHAN_SWEEP_STALE_MS` (TTL + 1h). The current process's active generation is always skipped. Do NOT revert to the bare-prefix sweep — it deleted a concurrently-running sibling Vite process's multi-GB active artifacts.
...
- Reverted Batch Stop to unscoped (audit F5) — a stale tab can cancel a newer run; or made `runId` strictly required without a migration — stale browser bundles break
