...
- Multi-strategy Universe jobs run selected strategies across a bounded `worker_threads` pool (`lib/finder/server/finder-universe-strategy-pool.ts`; worker entry `finder-universe-strategy-worker.ts`), reusing the genericized `runAssetOpportunityBatchSweep` coordinator. Each worker executes whole strategies through the UNCHANGED `runFinderUniverseExecution` core; backtest semantics are untouched. Locked by `tests/finder-universe-parallel.spec.ts`:
  ...
  - Worker count: `FINDER_UNIVERSE_WORKERS` env override (1 = sequential rollback lever; cap 32; bypasses the memory ceiling AND the Rust cap). Auto = min(strategy count, cores − 2, memory ceiling). The ceiling budgets 75%-RAM for one dataset copy per worker, estimated from `resolveUniverseMaxBarsPerSymbol` (bars × ~94 B) when the run's slice/interval bounds the bars (`date_range` with both bounds, or the `1`..`5` year slices), else the 100k-bar-cap worst case (~9 MB/symbol) — unbounded slices/intervals must keep the conservative estimate. With Rust preferred AND Rust actually executable, the AUTO value caps at 4; when settings force the TS engine (`hasCapabilityIndependentTypescriptRequirement` — e.g. exit-strategy override or slippage), the Rust server serializes nothing and the cap is skipped. The env override intentionally bypasses both.
