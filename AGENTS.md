...
- **Mine Prediction diagnostic** (`POST /api/batch-backtest/mine-prediction`): a fifth analysis route alongside Mine/Stability/PortfolioFit. It re-runs `runPreparedBatchSyntheticStateMiner` at sampled historical bars to measure whether Mine's analog `expectedForwardReturnPct` correlates with realized forward return (rank IC), with hit-rate / confidence-calibration / lift-correlation / per-asset breakdown / sign-aware verdict. The engine lives in `lib/batch-backtest/batch-mine-prediction-engine.ts` (pure leaf — no `lightweight-charts`). Three load-bearing contracts:
  - **READ-ONLY on artifacts**: like Mine Prediction, it does NOT call `releaseLastResults`. Mine/Stability/PortfolioFit can still run after it (and vice versa) within the TTL window. Do NOT add a release — it would break the "run Mine Prediction then Mine" sequence.
  - **Optional verdict-bar date window** (`sampleFromSec` / `sampleToSec`, unix seconds, null = full history): lets the UI run regime-specific tests (e.g. 2022 bear market) by filtering which historical bars are sampled. Surfaced in the report header as `window=YYYY-MM-DD..YYYY-MM-DD`.
  - **Direction filter** (`directionFilter: "both" | "long" | "short"`, default `"both"`): when set, scoring excludes the other direction's verdicts while retaining INCONCLUSIVE bars as the baseline. Critical for direction-biased strategies — including counter-predictive SHORT verdicts in a long-only strategy's IC drags the aggregate toward zero and corrupts pair-universe narrowing done off the result. Surfaced in the header as `direction=long`.
  - The endpoint streams `start` → `progress` (per-asset) → `bar` (throttled per-asset, at 0/25/50/75/100%) → `done`/`fatal`. The engine is heavy (~O(history × samples × pairs)); per-bar progress prevents the UI looking frozen on slow assets.
  - CLI: `npm run diagnose:mine-prediction` (`scripts/diagnose-mine-prediction.ts`) with `--sample-from` / `--sample-to` / `--direction-filter` / `--backtest-settings-file` flags for research use.
- **OPEN_SCORE USD Replay** (`POST /api/batch-backtest/open-score-usd`): a sixth analysis route that reconstructs historical OPEN_SCORE decision events from retained artifacts and measures whether each selector arm (TOP_RAW / TOP_ADJUSTED / TOP_MEAN / MAX_ACTIVE / MAX_SUBMITTED / MAX_RETAINED on the long side, MAX_ACTIVE_REVERSION on the short side) beats a uniform-random control at fixed forward horizons. The engine lives in `lib/batch-backtest/batch-open-score-usd-replay-engine.ts` (pure leaf — no `lightweight-charts`). Four load-bearing contracts:
...
- **F4 Disconnect-safe NDJSON streams**: all Batch streaming handlers wrap `beginNdjsonStream` in `createDisconnectSafeStream` (`lib/vite-http-utils.ts`). A disconnected Run keeps executing because `/status` can reattach and recover rows. Mine, Stability Mine, Portfolio Fit, and Mine Prediction cancel on disconnect because their streamed result is not recoverable; retained Batch artifacts remain available for retry. Do NOT pass `stream.write` directly to a long-running process function; use the wrapper's `.write`/`.end`.
