...
- The Finder **Symbol Universe** scope has two execution paths, gated by the `finderUniverseExecutionMode` setting (`Settings → Backend Engine → Finder Universe Execution Mode`):
  - `"browser"`: the original in-tab path via `FinderManager.runUniverseFinder` (`lib/finder-manager.ts`).
  - `"server"`: default; runs in the Vite dev server via `lib/finder/server/finder-vite-plugin.ts`, mirroring the Batch plugin's owner-lock + NDJSON stream pattern. See `docs/finder-server-side.md`. (Tab-reload reattach is NOT yet wired into the Finder browser path, unlike Batch.)
- Scope: applies to Symbol Universe only. The current-chart Finder always runs in-tab. Polymarket scoring is rejected by Universe mode regardless of path. v1 runs a single selected entry strategy per server request; multiple entry strategies fall back to the in-tab path.
...
- **OOS pass runs in-browser in v1**: the server path runs IS and returns survivors; `FinderManager.runUniverseFinder` feeds the server survivors into the SAME shared `applyUniverseOosValidationIfNeeded` tail as the in-tab path (do NOT early-return after `runUniverseFinderServer` — that skips OOS). Datasets are fetched client-side via `loadUniverseDataset` (cache-warm on `universeDatasetCache`). Server-side OOS is a documented follow-up.
...
- **Data slicing MUST be applied in the server loader wrapper**: the browser `FinderManager.runUniverseFinder` wraps `loadUniverseDataset` with `sliceFinderDataWindow(data, options.dataSlice)` before evaluation. The HTTP handler's `loadDatasetWithSlice` does the same; without it, browser/server results diverge for half-window / OOS / data-slice runs.
- **Stop MUST POST `/api/finder/stop` in server mode**: the Finder Stop button does both `this.isCancelled = true` (for the in-tab OOS pass) AND `POST /api/finder/stop` when `isRunning && isUniverseScope() && finderUniverseExecutionMode === 'server'`. The server runner cancels via ownership loss + abort, not via the browser's `isCancelled` flag.
- **`useRustEnginePreference` MUST thread through to `executeBacktest`**: the universe runner's `executeBacktest` context carries `useRustEnginePreference` (undefined in browser; set from the request body server-side). Without it, server-side Finder silently uses TS even when Rust is enabled (the documented Rust-engine trap).
- **Browser fallback on no dev server**: `FinderManager.runUniverseFinderServer` returns `null` on a 404/405 from `/api/finder/universe-run` (e.g. `vite preview`), and the caller falls through to the in-tab path. Do not remove this fallback or `vite preview` breaks.
- **Server-side import hygiene (same bundle trap as Batch)**: `lib/finder/server/finder-vite-plugin.ts` is imported by `vite.config.ts`. Do NOT import from `lib/finder-manager.ts`, `lib/data-manager.ts`, `lib/settings-manager.ts`, or anything transitively reaching `lib/constants.ts` or `lib/chart-manager.ts` (both pull `lightweight-charts`, ESM-only). The server loader imports only leaf modules (`data-fetcher`, `data-cache`, the synthetic-pair disk cache). Symptom of breakage: dev server fails to start with `Failed to resolve "lightweight-charts". This package is ESM only`.
- New setting id `finderUniverseExecutionMode` is registered in BOTH DOM-contract systems (`BACKTEST_DOM_SETTING_IDS` + `BACKTEST_SETTINGS_DOM_CONTRACTS`), with parser `"finderUniverseExecutionMode"` (case-sensitive; do NOT use the generic `"string"` parser which uppercases).
...
- Validation habit after Server-Side Finder changes:
  ...
  - `..\..\..\node_modules\.bin\esno tests\feature-dom-contracts.spec.ts`
  - `..\..\..\node_modules\.bin\esno tests\backtest-settings-id-parity.spec.ts`
  - Manual smoke (server-side): start `NODE_OPTIONS=--max-old-space-size=16384 npm run dev`, run a 50-symbol universe in browser mode (regression) then server-side mode (parity), then a 400-symbol universe in server-side mode (the goal). Confirm: progress streams, Stop works, Copy Diagnostics works, server/browser parity on survivors, OOS gate still filters server-mode survivors.
