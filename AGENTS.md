...
- **The `row` sent over the wire must NEVER contain array fields** (`data`, `signals`, `result.trades`). Use `toScalarRow(...)` (`lib/batch-backtest/batch-backtest-stream-types.ts`) before emitting a `symbol` event. The server retains the heavy arrays for Mine Timing in `lastResults` (module-scope on the plugin). This contract is what keeps the browser tab bounded regardless of pair count.
...
- **Artifact TTL**: the plugin holds `lastResults` for Mine Timing until Mine completes OR a new Run starts OR a 10-minute TTL fires (`DEFAULT_ARTIFACT_RETENTION_MS` in `batch-backtest-vite-plugin.ts`). The TTL is the defense-in-depth the browser path got for free via tab reload. Do NOT remove it without a replacement.
