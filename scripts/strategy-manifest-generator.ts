...
function getStrategyManifestPaths(root: string = repoRoot): StrategyManifestPaths {
    ...
    return {
        ...
        manifestEagerPath: path.join(resolvedRoot, "lib", "strategies", "manifest-eager.ts"),
        manifestMetaPath: path.join(resolvedRoot, "lib", "strategies", "manifest-meta.ts"),
        manifestSummaryPath: path.join(resolvedRoot, "lib", "strategies", "manifest-summary.ts"),
...
const manifestEagerPath = defaultPaths.manifestEagerPath;
const manifestMetaPath = defaultPaths.manifestMetaPath;
const manifestSummaryPath = defaultPaths.manifestSummaryPath;
...
export function collectStrategyModuleDefinitions(
    ...
    const existingOrder = [
        readCurrentManifestOrder(paths.manifestEagerPath),
        readCurrentManifestOrder(paths.manifestPath),
        readCurrentManifestOrder(paths.manifestKeysPath),
...
export function generateStrategyManifestSource(
    ...
    const keyLines = resolvedDefinitions.map(
        (definition) => `    builtInStrategyMetaByKey["${definition.key}"],`
    );
...
export function generateStrategyManifestEagerSource(
    definitions?: readonly StrategyModuleDefinition[],
    options: StrategyManifestGenerationOptions = {}
): string {
    const resolvedDefinitions = definitions ?? collectStrategyModuleDefinitions(options);
...
export function getStrategyManifestEagerPath(): string {
    return manifestEagerPath;
}
...
export function generateStrategySummarySource(
    definitions?: readonly StrategyModuleDefinition[],
    options: StrategyManifestGenerationOptions = {}
): string {
    const resolvedDefinitions = definitions ?? collectStrategyModuleDefinitions(options);
...
export function getStrategySummaryPath(): string {
    return manifestSummaryPath;
}
...
export function syncStrategyManifest(
    ...
    writeFileSync(paths.manifestEagerPath, generateStrategyManifestEagerSource(definitions, options), "utf8");
    writeFileSync(paths.manifestMetaPath, generateStrategyMetaSource(definitions, options), "utf8");
    writeFileSync(paths.manifestSummaryPath, generateStrategySummarySource(definitions, options), "utf8");
