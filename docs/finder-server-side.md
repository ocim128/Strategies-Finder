...
## Monthly Rank Replay (Symbol Universe submode)

Monthly Rank Replay is a **submode of Symbol Universe**, not a separate scope.
Enable it with the `Monthly Rank Replay` toggle in the Symbol Universe section
and configure three inputs: `From Year` (first scheduled checkpoint is January
of that year, UTC), `Eval Window Bars (L)` (trailing scored historical bars per
symbol), and `Forward Bars (H)` (forward scored bars per frozen winner). The
browser POSTs the SAME `POST /api/finder/universe-run` route; the server
detects `options.monthlyRankReplay` and dispatches the replay job INSTEAD of
the ordinary universe loop (no data slice, no OOS pass, no Rust capability
probing). Malformed explicit replay options return 400 before ownership is
acquired.

What the job does, per UTC month boundary T starting at January of `From Year`:

1. Score the last L closed bars per symbol (a bar is closed when
   `barOpenTime + intervalDuration <= T`). Earlier closed bars feed indicator
   warmup only — account state and statistics start flat at the scored start
   via the engine's `scoredRange` contract (`BacktestRunOptions.scoredRange` in
   `lib/strategies/backtest/backtest-engine.ts`).
2. Evaluate every candidate from ONE frozen seeded pool (same generation as
   ordinary Universe's plan builder) across the checkpoint's RETAINED symbol
   set (see coverage below). A candidate is eligible only after every retained
   symbol evaluates — valid no-trades count as evaluated; partial candidates
   never rank.
3. Select rank #1 independently for every historical Universe sort except the
   OOS-dependent Window Stability Score, preserving each sort's formula and
...
4. Forward-test each DISTINCT configuration the sorts need — every sort's
   ...
   price is ever used). Sorts choosing the same configuration share one
   forward evaluation per checkpoint. Synthetic pairs reduce through the
   existing pair-neutral transform; per-symbol outcomes are equally weighted
   inside a window and each sort summarizes only its own valid checkpoints.

   The Robust Universe Score row replays the sort's OWN single-sort
   dependency: the ordinary robust sort never computes Composite Edge Ratio,
...
ranges and warmup counts. At each checkpoint, a symbol with insufficient
closed scored history, an incomplete forward horizon, or a failed load is
EXCLUDED from that checkpoint and named — with its reason — on the checkpoint
record (`retainedSymbols` + `excludedSymbols`), in the UI's Data coverage
section, and in Copy Results. Window means use the checkpoint's retained set,
so symbol membership grows as late listings accrue bars; summaries compare
months with potentially different membership, which the per-checkpoint counts
make visible. A checkpoint is unavailable only when NO symbol is retained.
The schedule spans the run's overall data extent (latest closed bar across
loaded series), not the earliest-ending symbol. Cross-symbol strategies, Polymarket scoring,
strategy-timeframe resampling, combined direction, and non-fixed sizing are
rejected explicitly. Every backtest runs the TypeScript engine (`engine:
typescript`), even when ordinary Finder prefers Rust.

Wire contract: `jobKind: "monthly_rank_replay"` on `/status`; stream events
`replay_start` / `replay_progress` / `replay_checkpoint` / `replay_done` /
`replay_fatal` (all scalar-only — `assertReplayCheckpointIsScalar` guards the
checkpoint payloads). The terminal snapshot carries the full report on
`terminalReplay` (never in `terminalCandidates`); running polls return counts
only. Stop, runId scoping, disconnect-safe streaming, loopback authorization,
and reload reattach reuse the existing job lifecycle unchanged. The browser
persists only the replay summary + runId in its latest-results envelope and
recovers full detail from `/status` after a reload; if the server state is
gone the UI labels detail unavailable and Copy emits the identified summary.
The output describes "forward outcomes of monthly historical rank #1" — it is
a retrospective replay of the current strategy library and symbol list, not
evidence of live profitability, and overlapping H-bar windows are not
calendar-month returns.

Replay debug events: `finder.replay.start`, `finder.replay.complete`,
`finder.replay.cancelled`, and `finder.replay.fatal` — counts, timings, and
errors only.
