...
- the exported const matches the manifest import name
- the strategy is registered in `lib/strategies/manifest.ts`
...
1. Pick a stable key and keep the file name, exported const, and manifest key aligned.
...
- exported const name matches the manifest import
...
- manifest import and entry were added in `lib/strategies/manifest.ts`
...
- adding a strategy file but forgetting the manifest entry
