...
exact report text is generated by
`lib/batch-backtest/batch-open-score-usd-replay-engine.ts`; keep the opaque
`reportLines` copy contract intact when adding an arm.
