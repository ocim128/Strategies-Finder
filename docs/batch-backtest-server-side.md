...
skips an event when the selected asset's last two completed returns were both
negative, and `regimeFloor` skips when the mean of the last ten incumbent
returns is negative. Each gate reports the gated vs ungated mean, the value
the gate skipped, and ten-block stability. The standalone
`scripts/take-skip-eval.ts` evaluator (locked by
`tests/take-skip-eval.spec.ts`) reproduces the same lines offline from an
archived ledger. The exact report text is generated by the engine; keep the
opaque `reportLines` copy contract intact when adding an arm.

### Cap-tilt weighting (`capTiltWeight`)

The OPEN_SCORE USD rerun accepts `capTiltWeight: "off" | "smallBase2x" |
"largeBase2x"` (default `"off"`, selectable next to the horizons input). When
...
quote's, `largeBase2x` when **higher** (data source: the Download MarketCap
