...
/** Build one scalar research summary per symbol from the complete result set. */
export function buildAssetOpportunityPairSummaries(
    results: FinderAssetOpportunityResult[],
): AssetOpportunityPairSummaryRow[] {
    interface PairAggregate {
        candidateCount: number;
        profitableCount: number;
        netProfitPercents: number[];
        avgTrades: number[];
        topNetProfit: number;
    ...
    for (const result of results) {
        let aggregate = aggregates.get(result.symbol);
        if (!aggregate) {
            aggregate = {
                candidateCount: 0,
                profitableCount: 0,
        ...
        if (result.selectionResult.netProfit > 0) aggregate.profitableCount += 1;
        if (Number.isFinite(result.selectionResult.netProfitPercent)) {
            aggregate.netProfitPercents.push(result.selectionResult.netProfitPercent);
        }
        if (Number.isFinite(result.selectionResult.avgTrade)) {
            aggregate.avgTrades.push(result.selectionResult.avgTrade);
        }
        if (Number.isFinite(result.selectionResult.netProfit)
            && (!Number.isFinite(aggregate.topNetProfit) || result.selectionResult.netProfit > aggregate.topNetProfit)) {
            aggregate.topNetProfit = result.selectionResult.netProfit;
        }

        for (const horizon of result.oosHorizonMetrics?.horizons ?? []) {
            if (Number.isFinite(horizon.bars)) horizonBars.add(horizon.bars);
            if (horizon.sampleSize < 1
                || horizon.averagePnlPercent === null
                || !Number.isFinite(horizon.averagePnlPercent)) {
                continue;
            }
            const values = aggregate.forwardByHorizon.get(horizon.bars) ?? [];
    ...
    return [...aggregates.entries()]
        ...
        .map(([symbol, aggregate]) => {
            const p25 = quantile(aggregate.netProfitPercents, 0.25);
            const p75 = quantile(aggregate.netProfitPercents, 0.75);
            const forwardPnlPercentByHorizon: Record<number, number | null> = {};
            for (const bars of [...horizonBars].sort((left, right) => left - right)) {
                const values = aggregate.forwardByHorizon.get(bars);
                forwardPnlPercentByHorizon[bars] = values && values.length > 0
            ...
            return {
                ...
                profitableShare: aggregate.profitableCount / aggregate.candidateCount,
                medianNetProfitPercent: quantile(aggregate.netProfitPercents, 0.5),
                netProfitP75MinusP25: p75 - p25,
