...
import type { LiveTradeOrderType, LiveTradeSubmitRequest, LiveTradeSubmitResponse } from "./execution-lab-model";
import {
...
export async function submitLiveTradeToExecutor(
    ...
    if (request.action === "entry" && request.stakeUsd > config.maxStakeUsd) {
        return buildLiveTradeFailureResponse({
            requestId: request.requestId,
            status: "rejected",
