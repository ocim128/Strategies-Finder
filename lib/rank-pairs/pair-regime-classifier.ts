...
export function classifyPairRegime(bars: OHLCVData[]): PairRegimeResult {
    ...
    const recentOppositeFull =
        recentStrong &&
        full.normalizedDrift !== null &&
        recentNormalizedDrift !== null &&
        Math.sign(full.normalizedDrift) !== Math.sign(recentNormalizedDrift);

    let structure: PairStructure;
    if (recentOppositeFull && driftAbs !== null && driftAbs >= DRIFT_THRESHOLD) {
        structure = "REVERSAL";
    } else if (recentStrong && !fullIsTrending) {
