...
export class TradeLedgerSweepService {
    ...
    private async startRun(): Promise<void> {
        ...
        try {
            ...
            await consumeNdjsonStream<LedgerSweepStreamEvent>(response.body, {
                ...
                onDiagnostics: (event) => { if (isTradeLedgerSweepRunCurrent(this.activeServerRunId, event.runId)) { this.diagnostics = event.entry; this.getDom().tradeLedgerSweepDiagnostics.textContent = formatTradeLedgerSweepDiagnostics(event.entry); } },
    ...
    private async copyDiagnostics(): Promise<void> {
        if (this.diagnostics !== null) await copyToClipboard(formatTradeLedgerSweepDiagnostics(this.diagnostics));
    }
}
