...
export interface PairListProvenanceV1 {
    ...
    /** SHA-256 provenance added for successor campaign registrations. */
...
export function generateBalancedPairList(options: BalancedPairListOptions): BalancedPairListResult {
    ...
    const provenance: PairListProvenanceV1 = {
        ...
        emittedPairListHash,
        executionOrderSha256: sha256LineList(normalizedEmitted),
        sortedSetSha256: sha256LineList([...normalizedEmitted].sort((left, right) => left < right ? -1 : left > right ? 1 : 0)),
...
// Synchronous browser-safe SHA-256 for deterministic generator provenance.
// The generator is imported by the browser Batch service, so Node's crypto
// module cannot be used for these two registration hashes.
function sha256Hex(value: string): string {
    ...
    const rotateRight = (word: number, amount: number): number => (word >>> amount) | (word << (32 - amount));
    const state = [...initial];
