...
async function handleRun(): Promise<void> {
    ...
    try {
        ...
        if (primaryEntry.result.status !== "success") {
            dom.statusSpan.textContent = primaryEntry.result.errorMessage ?? "Monte Carlo could not complete";
            return;
        }
...
function refreshSimulationCapHint(): void {
    ...
    dom.simulationCapHint.textContent = `Safe cap: ${chartSafeCap.toLocaleString()} sims across ${scenarioCount} active method set${scenarioCount === 1 ? "" : "s"}.`;
}
...
function formatCompletionStatus(
    ...
    if (scenarioCount > 1) {
        if (requestedSimulations > effectiveSimulations) {
            baseMessage = `Completed ${effectiveSimulations.toLocaleString()} sims per scenario across ${scenarioCount} scenarios (${totalEffective.toLocaleString()} total) in ${executionTimeMs}ms. Requested ${requestedSimulations.toLocaleString()} per scenario.`;
        } else {
            baseMessage = `Completed ${effectiveSimulations.toLocaleString()} sims per scenario across ${scenarioCount} scenarios (${totalEffective.toLocaleString()} total) in ${executionTimeMs}ms.`;
        }
    } else if (requestedSimulations > effectiveSimulations) {
        baseMessage = `Completed ${effectiveSimulations.toLocaleString()} capped simulations in ${executionTimeMs}ms (requested ${requestedSimulations.toLocaleString()}).`;
    } else {
        baseMessage = `Completed ${totalEffective.toLocaleString()} simulations in ${executionTimeMs}ms.`;
    }
    return baseMessage;
}
...
function refreshActionAvailability(): void {
    ...
    const chartAvailable = !!state.currentBacktestResult;
