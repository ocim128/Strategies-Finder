...
export const vwap_entropy_gradient_breakout: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams) => {
        ...
        return createSignalLoop(cleanData, [vwap, entropy, gradPctl], (i) => {
            ...
            // Buy: entropy < maxEntropy, price above VWAP, close location gradient percentile > 0.75
            if (currentEntropy < maxEnt && close > currentVwap && currentGradPctl > 0.75) {
                return createBuySignal(cleanData, i, `VWAP Entropy Breakout Buy: Entropy ${currentEntropy.toFixed(2)}, GradPctl ${currentGradPctl.toFixed(2)}`);
            }
            // Sell: entropy < maxEntropy, price below VWAP, close location gradient percentile < 0.25
            if (currentEntropy < maxEnt && close < currentVwap && currentGradPctl < 0.25) {
                return createSellSignal(cleanData, i, `VWAP Entropy Breakout Sell: Entropy ${currentEntropy.toFixed(2)}, GradPctl ${currentGradPctl.toFixed(2)}`);
            }
