...
export const bear_hunter_v5: Strategy = {
    name: "Bear Hunter v5",
    description: "Banker crash shield: bank half at +5%, move remaining risk to breakeven, and trail remainder tightly.",
    defaultParams: {
        fastPeriod: 34,
        slowPeriod: 89,
        macroSmaPeriod: 200,
        ...
        for (let i = 1; i < cleanData.length; i++) {
            ...
            if (!inShort) {
                ...
                if (confirmStreak >= cfg.confirmBars) {
                    ...
                    trailActive = false;
                    confirmStreak = 0;
                    signals.push(createSellSignal(cleanData, i, "Bear v5 wipeout short entry"));
                }
                continue;
            }
            ...
            if (!bankedHalf && profitPct >= cfg.bankTriggerProfitPct) {
                const bankFraction = cfg.bankFractionPct / 100;
                signals.push(createBuySignal(cleanData, i, "Bear v5 bank half", bankFraction));
                bankedHalf = true;
            ...
            const trailStop = trailActive && closes[i] >= bestPrice + cfg.remainingTrailAtr * atrNow;
            ...
            if (hardStop || trailStop || timeout || regimeExit) {
                const reason = hardStop
                    ? "Bear v5 hard stop"
                    : trailStop
                        ? "Bear v5 remaining trail stop"
                        : timeout
                            ? "Bear v5 max-hold exit"
                            : "Bear v5 regime cover";
