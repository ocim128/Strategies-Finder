...
export const tight_range_efficiency_direction: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams) => {
        ...
        return createSignalLoop(cleanData, [rangePctl, efficiency], (i) => {
            ...
            if (ca > 0) {
                return createBuySignal(cleanData, i, `Tight range pctl ${rp.toFixed(2)} eff ${er.toFixed(2)} bullish drift`);
            }
            if (ca < 0) {
                return createSellSignal(cleanData, i, `Tight range pctl ${rp.toFixed(2)} eff ${er.toFixed(2)} bearish drift`);
            }
            return null;
        });
    },
