...
function percentileLinear(values: number[], percentile: number): number {
    ...
    if (lo === Math.ceil(idx)) return sorted[lo];
    const weight = idx - lo;
    return sorted[lo] * (1 - weight) + sorted[lo + 1] * weight;
}

function makeClusters(centroids: number[]): Cluster[] {
    return centroids.map(c => ({ centroid: c, values: [], factors: [] }));
}
...
function kMeans1d(values: number[], factors: number[], maxIter: number): Cluster[] {
    ...
    ].map(c => (Number.isFinite(c) ? c : 0));

    let clusters = makeClusters(centroids);

    for (let iter = 0; iter < maxIter; iter++) {
        clusters = makeClusters(centroids);
        ...
        for (let i = 0; i < values.length; i++) {
            ...
            for (let c = 0; c < centroids.length; c++) {
                ...
                if (d < bestDist) { bestIdx = c; bestDist = d; }
            }
        ...
        const newCentroids = centroids.map((prev, idx) => {
            const vals = clusters[idx].values;
            return vals.length === 0 ? prev : vals.reduce((a, v) => a + v, 0) / vals.length;
        });

        if (newCentroids.every((v, i) => Math.abs(v - centroids[i]) < 1e-8)) break;
...
// Shared core computation used by both execute and indicators
function computeCore(data: OHLCVData[], params: StrategyParams) {
    ...
    const states: SupertrendFactorState[] = factorValues.map(() => ({
        upper: 0, lower: 0, output: 0, perf: 0, trend: 1, initialized: false
    }));
    ...
    for (let i = 0; i < cleanData.length; i++) {
        ...
        for (let fIdx = 0; fIdx < factorValues.length; fIdx++) {
            ...
            if (!state.initialized) {
                state.upper = hl2; state.lower = hl2; state.output = hl2;
                state.perf = 0; state.trend = closes[i] >= hl2 ? 1 : 0;
        ...
        if (i >= warmupBars && i % kMeansInterval === 0) {
            ...
            if (Number.isFinite(perfAvg)) lastClusterPerf = perfAvg;
        }

        if (!Number.isFinite(targetFactor)) targetFactor = lastClusterFactor;

        const up = hl2 + atrVal * targetFactor;
        const dn = hl2 - atrVal * targetFactor;

        if (finalUpper === null || finalLower === null) {
            finalUpper = up; finalLower = dn;
        } else {
    ...
    return { cleanData, closes, direction, ts, lastClusterPerf, perfAlpha };
}
...
export const adaptive_supertrend_kmeans: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams): Signal[] => {
        ...
        const { cleanData, direction } = core;
    ...
    indicators: (data: OHLCVData[], params: StrategyParams): StrategyIndicator[] => {
        ...
        const { cleanData, closes, ts, lastClusterPerf, perfAlpha } = core;
