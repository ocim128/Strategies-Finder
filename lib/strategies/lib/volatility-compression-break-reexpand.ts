...
export const volatility_compression_break_reexpand: Strategy = {
    name: 'Volatility Compression Break Reexpand',
    description: 'Breaks previous bar range after compression only when short ATR re-expands on the breakout bar. Range lookback is fixed at 1 and ATR buffer is fixed at 0.',
    defaultParams: {
        compressionRatio: 0.7,
        atrExpansionMin: 1.15,
    },
    paramLabels: {
        compressionRatio: 'ATR Compression Ratio',
        atrExpansionMin: 'ATR Expansion Min',
    },
    execute: (data: OHLCVData[], params: StrategyParams): Signal[] => {
        ...
        const atrExpansionMin = clamp(params.atrExpansionMin ?? 1.15, 0.9, 3);
        ...
        for (let i = 1; i < cleanData.length; i++) {
            ...
            const atrExpansion = atrSCurr / atrSPrev;
            if (atrExpansion < atrExpansionMin) continue;

            const prevClose = closes[i - 1];
            const close = closes[i];
            if (prevClose <= prevUpper && close > prevUpper) {
                signals.push(createBuySignal(cleanData, i, 'VCB Reexpand Break Up'));
                continue;
            }

            if (prevClose >= prevLower && close < prevLower) {
                signals.push(createSellSignal(cleanData, i, 'VCB Reexpand Break Down'));
            }
        }
    ...
    metadata: {
        role: 'entry',
        direction: 'both',
        walkForwardParams: ['compressionRatio', 'atrExpansionMin'],
    },
};
