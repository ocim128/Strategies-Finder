...
export const vwap_outer_band_gradient_reversal: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams) => {
        ...
        return createSignalLoop(cleanData, [vwap, diffStdDev, closeLoc], (i) => {
            ...
            if (belowBand && currGrad > 0) {
                return createBuySignal(cleanData, i, `VWAP Band Rev Buy: Close ${close.toFixed(4)}, Band ${(currentVwap - mult * currentStd).toFixed(4)}, Grad ${currGrad.toFixed(4)}`);
            }
            // Sell: price above upper band, close location gradient negative
            if (aboveBand && currGrad < 0) {
                return createSellSignal(cleanData, i, `VWAP Band Rev Sell: Close ${close.toFixed(4)}, Band ${(currentVwap + mult * currentStd).toFixed(4)}, Grad ${currGrad.toFixed(4)}`);
            }
