...
function getQuietRangeInfo(unixSeconds: number, startHour: number, endHour: number): QuietRangeInfo {
    if (startHour === endHour) return { inRange: false, windowId: null };
    ...
    if (startHour < endHour) {
        ...
        return { inRange, windowId: inRange ? toUtcDateKey(unixSeconds) : null };
    }

    if (hour >= startHour) return { inRange: true, windowId: toUtcDateKey(unixSeconds) };
    if (hour < endHour) return { inRange: true, windowId: toUtcDateKey(unixSeconds - 86400) };
    return { inRange: false, windowId: null };
}

export const asian_session_breakout_v2: Strategy = {
    name: 'Asian Session Breakout v2',
    description: 'UTC quiet-range breakout with ATR protection and EMA trend filter (long-only above EMA, short-only below EMA).',
    defaultParams: {
        rangeStartHour: 0,
        rangeEndHour: 8,
        stopATR: 1.5,
    ...
    paramLabels: {
        ...
        emaTrendPeriod: 'EMA Trend Period'
    },
    execute: (data: OHLCVData[], params: StrategyParams): Signal[] => {
        ...
        const emaTrendPeriod = Math.max(20, Math.round(params.emaTrendPeriod ?? 200));
        ...
        const trendEma = calculateEMA(closes, emaTrendPeriod);
        ...
        for (let i = 0; i < cleanData.length; i++) {
            ...
            if (rangeInfo.inRange) {
                ...
                if (isNewRange) {
                    ...
                    if (position === 'long') {
                        signals.push(createSellSignal(cleanData, i, 'Asian v2 session reset'));
                        position = 'flat';
                        stopPrice = 0;
                        entryIndex = -1;
                    } else if (position === 'short') {
                        signals.push(createBuySignal(cleanData, i, 'Asian v2 session reset'));
                        position = 'flat';
                        stopPrice = 0;
                        entryIndex = -1;
                    }
            ...
            if (position === 'long') {
                ...
                if (stopHit) {
                    signals.push(createSellSignal(cleanData, i, 'Asian v2 long stop'));
                    position = 'flat';
                    stopPrice = 0;
                    entryIndex = -1;
                }
            ...
            if (position === 'short') {
                ...
                if (stopHit) {
                    signals.push(createBuySignal(cleanData, i, 'Asian v2 short stop'));
                    position = 'flat';
                    stopPrice = 0;
                    entryIndex = -1;
                }
            ...
            const atrNow = atr[i];
            const emaNow = trendEma[i];
            if (atrNow === null || atrNow <= 0 || emaNow === null) continue;

            const close = closes[i];
            const canLong = close > emaNow;
            const canShort = close < emaNow;

            if (close > readyRange.high && canLong) {
                signals.push(createBuySignal(cleanData, i, 'Asian v2 upside breakout + EMA filter'));
                position = 'long';
                stopPrice = close - (stopATR * atrNow);
                entryIndex = i;
                tradedWindowId = readyRange.windowId;
            } else if (close < readyRange.low && canShort) {
                signals.push(createSellSignal(cleanData, i, 'Asian v2 downside breakout + EMA filter'));
                position = 'short';
                stopPrice = close + (stopATR * atrNow);
                entryIndex = i;
                tradedWindowId = readyRange.windowId;
        ...
        if (position === 'long' && cleanData.length > 0) {
            signals.push(createSellSignal(cleanData, cleanData.length - 1, 'Asian v2 final close'));
        } else if (position === 'short' && cleanData.length > 0) {
            signals.push(createBuySignal(cleanData, cleanData.length - 1, 'Asian v2 final close'));
        }
    ...
    metadata: {
        role: 'entry',
        direction: 'both',
        walkForwardParams: ['rangeStartHour', 'rangeEndHour', 'stopATR', 'emaTrendPeriod']
    }
};
