...
export const vwap_squeeze_gradient_release: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams) => {
        ...
        return createSignalLoop(cleanData, [vwap, trPct, closeLoc], (i) => {
            ...
            if (prevPct < (p.compressionLimit as number) && currPct > 0.7 && close > currentVwap && currGrad > 0) {
                return createBuySignal(cleanData, i, `VWAP Squeeze Release Buy: PrevPct ${prevPct.toFixed(2)}, CurrPct ${currPct.toFixed(2)}, Grad ${currGrad.toFixed(4)}`);
            }
            ...
            if (prevPct < (p.compressionLimit as number) && currPct > 0.7 && close < currentVwap && currGrad < 0) {
                return createSellSignal(cleanData, i, `VWAP Squeeze Release Sell: PrevPct ${prevPct.toFixed(2)}, CurrPct ${currPct.toFixed(2)}, Grad ${currGrad.toFixed(4)}`);
            }
