...
function normalizeParams(params: StrategyParams): StrategyParams {
    return {
        ...
        skewMin: Number(params.skewMin ?? 0.2),
    };
}

export const vwap_skew_gradient_alignment: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams) => {
        ...
        return createSignalLoop(cleanData, [vwap, skewness, closeLoc], (i) => {
            ...
            const minSkew = p.skewMin as number;

            // Buy: True range skewness > skewMin, price above VWAP, close location gradient positive
            if (currentSkew > minSkew && close > currentVwap && currGrad > 0) {
                return createBuySignal(cleanData, i, `VWAP Skew Align Buy: Skew ${currentSkew.toFixed(2)}, Grad ${currGrad.toFixed(4)}`);
            }
            // Sell: True range skewness < -skewMin, price below VWAP, close location gradient negative
            if (currentSkew < -minSkew && close < currentVwap && currGrad < 0) {
                return createSellSignal(cleanData, i, `VWAP Skew Align Sell: Skew ${currentSkew.toFixed(2)}, Grad ${currGrad.toFixed(4)}`);
            }
