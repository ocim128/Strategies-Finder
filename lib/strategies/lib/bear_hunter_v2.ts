...
function normalizeConfig(params: StrategyParams): BearHunterV2Config {
    ...
    return {
        ...
        minHoldBars: Math.max(1, Math.round(params.minHoldBars ?? 3)),
        cooldownBars: Math.max(0, Math.round(params.cooldownBars ?? 8)),
        atrPeriod: Math.max(5, Math.round(params.atrPeriod ?? 14)),
        stopAtr: Math.max(0.3, params.stopAtr ?? 1.35),
        trailAtr: Math.max(0.5, params.trailAtr ?? 1.65),
        maxHoldBars: Math.max(4, Math.round(params.maxHoldBars ?? 30)),
    };
}

export const bear_hunter_v2: Strategy = {
    name: "Bear Hunter v2",
    description: "Short-only crash shield with macro bear gate, oversold entry guard, and fast V-shape recovery exits.",
    defaultParams: {
        fastPeriod: 34,
        slowPeriod: 89,
        macroSmaPeriod: 200,
        ...
        for (let i = 1; i < cleanData.length; i++) {
            ...
            if (!inShort) {
                ...
                if (confirmStreak >= cfg.confirmBars) {
                    ...
                    bestPrice = closes[i];
                    confirmStreak = 0;
                    signals.push(createSellSignal(cleanData, i, "Bear v2 wipeout short entry"));
                }
                continue;
            }
            ...
            const hardStop = closes[i] >= entryPrice + cfg.stopAtr * atrNow;
            ...
            if (hardStop || trailStop || timeout || regimeExit) {
                const reason = hardStop
                    ? "Bear v2 hard stop"
                    : trailStop
                        ? "Bear v2 trailing stop"
                        : timeout
                            ? "Bear v2 max-hold exit"
                            : "Bear v2 regime recovery cover";
