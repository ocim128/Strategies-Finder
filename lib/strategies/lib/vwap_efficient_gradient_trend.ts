...
export const vwap_efficient_gradient_trend: Strategy = {
    ...
    execute: (data: OHLCVData[], params: StrategyParams) => {
        ...
        return createSignalLoop(cleanData, [vwap, er, closeLoc], (i) => {
            ...
            // Buy: Price above VWAP, efficiency > minEfficiency, close location gradient positive
            if (close > currentVwap && currentEr > (p.minEfficiency as number) && currGrad > 0) {
                return createBuySignal(cleanData, i, `VWAP Eff Trend Buy: Close ${close.toFixed(4)}, VWAP ${currentVwap.toFixed(4)}, ER ${currentEr.toFixed(2)}`);
            }
            // Sell: Price below VWAP, efficiency > minEfficiency, close location gradient negative
            if (close < currentVwap && currentEr > (p.minEfficiency as number) && currGrad < 0) {
                return createSellSignal(cleanData, i, `VWAP Eff Trend Sell: Close ${close.toFixed(4)}, VWAP ${currentVwap.toFixed(4)}, ER ${currentEr.toFixed(2)}`);
            }
