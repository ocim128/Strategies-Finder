...
function normalizeParams(params: StrategyParams): StrategyParams {
	return {
		...
		minKellyFraction: Number(params.minKellyFraction ?? 0.15),
	};
}

export const kelly_autocorrelation_probability_reversion: Strategy = {
	...
	executePrepared: (preparedData: unknown, params: StrategyParams, data: OHLCVData[]) => {
		...
		return createSignalLoop(cleanData, [zscore, autocorr], (i) => {
			...
			if (kelly > minKellyFraction) {
				if (z < -1.5) {
					return createBuySignal(cleanData, i, `Autocorr Kelly buy: Z ${z.toFixed(2)}, Autocorr ${ac.toFixed(2)}, Kelly ${kelly.toFixed(3)} > ${minKellyFraction}`);
				}
				if (z > 1.5) {
					return createSellSignal(cleanData, i, `Autocorr Kelly sell: Z ${z.toFixed(2)}, Autocorr ${ac.toFixed(2)}, Kelly ${kelly.toFixed(3)} > ${minKellyFraction}`);
				}
			}
			return null;
		});
	...
	execute: (data, params) =>
		kelly_autocorrelation_probability_reversion.executePrepared!(
			kelly_autocorrelation_probability_reversion.prepareFinderData!(data),
			params,
			data
		),
	metadata: {
