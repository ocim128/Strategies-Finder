...
function normalizeParams(params: StrategyParams): StrategyParams {
	return {
		...
		kellyMinFraction: Number(params.kellyMinFraction ?? 0.25),
	};
}
...
export const kelly_zscore_fractional_reversion: Strategy = {
	...
	executePrepared: (preparedData: unknown, params: StrategyParams, data: OHLCVData[]) => {
		...
		const kellyMinFraction = p.kellyMinFraction as number;
		...
		return createSignalLoop(cleanData, [zscore], (i) => {
			...
			if (z < -1.8) {
				...
				if (kelly > kellyMinFraction) {
					return createBuySignal(cleanData, i, `Kelly stretch buy: Z ${z.toFixed(2)}, winProb ${winProb.toFixed(3)}, Kelly ${kelly.toFixed(3)} > ${kellyMinFraction}`);
				}
			} else if (z > 1.8) {
				const winProb = normalCDF(z);
				const kelly = 2 * winProb - 1;
				if (kelly > kellyMinFraction) {
					return createSellSignal(cleanData, i, `Kelly stretch sell: Z ${z.toFixed(2)}, winProb ${winProb.toFixed(3)}, Kelly ${kelly.toFixed(3)} > ${kellyMinFraction}`);
				}
			}
			return null;
		});
	...
	execute: (data, params) =>
		kelly_zscore_fractional_reversion.executePrepared!(
			kelly_zscore_fractional_reversion.prepareFinderData!(data),
			params,
			data
		),
	metadata: {
