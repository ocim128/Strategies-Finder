...
export function normalizeBacktestSettings(settings?: BacktestSettings): NormalizedSettings {
    ...
    return {
        ...
        riskProbationBars: Math.max(0, Math.round(toNumberOr(settings?.riskProbationBars, 0))),
        riskProbationMinR: Math.max(0, toNumberOr(settings?.riskProbationMinR, 0)),
        riskProbationCooldownBars: Math.max(0, Math.round(toNumberOr(settings?.riskProbationCooldownBars, 0))),
        riskProbationEnabled: settings?.riskProbationEnabled ?? false,
        riskLossStreakConsecutive: Math.max(0, Math.round(toNumberOr(settings?.riskLossStreakConsecutive, 0))),
        riskLossStreakWindowSize: Math.max(0, Math.round(toNumberOr(settings?.riskLossStreakWindowSize, 0))),
